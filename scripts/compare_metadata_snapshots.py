...
def main():
    ...
    with open(args.local_file, "r", encoding="utf-8") as f:
        local = json.load(f)
    with open(args.service_file, "r", encoding="utf-8") as f:
        service = json.load(f)
    ...
    lines = []
    lines.append("metadata snapshot comparison: local vs service")
    lines.append("")
    ...
    lines.append("")
    lines.append("task-level signals (final snapshot)")
