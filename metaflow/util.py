...
def dict_to_cli_args(params, skip_local_config_file=False):
    ...
    for k, v in params.items():
        ...
        # We need special handling for 'with' since it is a reserved keyword in
        # Python, so we call it 'decospecs' in click args.
        if k == "decospecs":
            k = "with"
        ...
        if k in ("config", "config_value"):
            # Special handling here since we gather them all in one option but actually
            # need to send them one at a time using --config-value <name> kv.<name>.
            # Note it can be either config or config_value depending on click
            # processing order.
            for config_name in v.keys():
                yield "--config-value"
                yield to_unicode(config_name)
